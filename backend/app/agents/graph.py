...
async def run_agent_pipeline_with_tools(
    ...
    system_prompt = (
        ...
        "1. **查询工单**：用户说'我的工单'、'工单状态'、'查工单'、'有哪些工单'时 → 调用 get_my_tickets\n"
        "   - 工具支持按状态、分类、优先级筛选\n"
        "   - 系统限制每次最多返回5条工单，回复时必须说明此上限\n"
        "2. **搜索知识库**：用户问任何具体问题时 → 调用 search_knowledge\n\n"
        "【回复规则】\n"
        "- 查询工单时，必须在回复中提及'系统限制最多显示5条'\n"
        "- 如果用户未指定筛选条件，返回最新5条工单\n"
        "- 工具返回的数据是真实数据，直接引用，不要修改\n"
